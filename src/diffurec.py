...
class Diffu_xstart(nn.Module):
    ...
    def forward(self, rep_item, x_t, t, mask_seq):
        ...
        # k = 2.0
        # lam = 0.5
        # lambda_uncertainty = (Weibull(torch.tensor(k), torch.tensor(lam)).sample(rep_item.shape)-1).to(rep_item.device)
...
class DiffuRec(nn.Module):
    ...
    def q_sample(self, x_start, t, noise=None, mask=None):
        ...
        if noise is None:
            ...
            k = 2.0
            lam = 0.5
            noise = (Weibull(torch.tensor(k), torch.tensor(lam)).sample(x_start.shape)-1).to(x_start.device)
    ...
    def p_sample(self, item_rep, noise_x_t, t, mask_seq):
        ...
        k = 2.0
        lam = 0.5
        noise = (Weibull(torch.tensor(k), torch.tensor(lam)).sample(noise_x_t.shape)-1).to(noise_x_t.device)
    ...
    def forward(self, item_rep, item_tag, mask_seq):        
        ...
        k = 2.0
        lam = 0.5
        noise = (Weibull(torch.tensor(k), torch.tensor(lam)).sample(item_tag.shape)-1).to(item_tag.device)

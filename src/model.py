...
class Att_Diffuse_model(nn.Module):
    ...
    #     # 使用归一化的交叉熵损失
    #     scores = self.diffu_rep_pre(rep_diffu)
    #     norm_scores = F.normalize(scores, p=2, dim=1)  # L2归一化
    #     return self.loss_ce(norm_scores, labels.squeeze(-1))
    ...
    #     # 对rep_diffu进行L2归一化
    #     rep_diffu_norm = F.normalize(rep_diffu, p=2, dim=1)
    #     scores = torch.matmul(rep_diffu_norm, F.normalize(self.item_embeddings.weight, p=2, dim=1).t())
    #     return scores
    ...
    def forward(self, sequence, tag, train_flag=True): 
        ...
        if train_flag:
            ...
            # 生成与 item_embeddings[:,-1, :] 形状相同的威布尔分布噪声
            noise_x_t = (Weibull(torch.tensor(k), torch.tensor(lam)).sample(item_embeddings[:, -1, :].shape)-1).to(
